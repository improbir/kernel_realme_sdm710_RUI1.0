...
static struct df_boost_drv df_boost_drv_g __read_mostly = {
	BOOST_DEV_INIT(df_boost_drv_g, DEVFREQ_MSM_GPUBW,
		       CONFIG_DEVFREQ_MSM_GPUBW_BOOST_FREQ_LOW)
};
...
static void __devfreq_boost_kick_flex(struct boost_dev *b)
{
	...
	if (!mod_delayed_work(b->wq_f, &b->flex_unboost,
			msecs_to_jiffies(flex_boost_gpu_duration))) {
...
static void devfreq_update_boosts(struct boost_dev *b, unsigned long state)
{
	...
	if (!test_bit(SCREEN_ON, &state)) {
		...
		df->min_freq = test_bit(FLEX_BOOST, &state) ?
			min(devfreq_boost_gpu_freq_low, df->max_freq) :
			df->profile->freq_table[0];
		df->min_freq = test_bit(INPUT_BOOST, &state) ?
			min(devfreq_boost_gpu_freq, df->max_freq) :
			df->profile->freq_table[0];
...
static int __init devfreq_boost_gpu_init(void)
{
	...
	for (i = 0; i < DEVFREQ_MAX_GPU; i++) {
		struct boost_dev *b = d->devices + i;
		b->wq_i = alloc_workqueue("devfreq_boost_gpu_wq_i", WQ_HIGHPRI, 0);
		b->wq_f = alloc_workqueue("devfreq_boost_gpu_wq_f", WQ_HIGHPRI, 0);
		b->wq_m = alloc_workqueue("devfreq_boost_gpu_wq_m", WQ_HIGHPRI, 0);
